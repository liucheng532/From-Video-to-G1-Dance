...
from robocup_lab.tasks.tracking.mdp import MotionCommand
...
def attach_onnx_metadata(env: ManagerBasedRLEnv, run_path: str, path: str, filename="policy.onnx") -> None:
    ...
    metadata = {
        ...
        "default_joint_pos": (env.scene["robot"].data.default_joint_pos_nominal.cpu().tolist()
                             if hasattr(env.scene["robot"].data, "default_joint_pos_nominal")
                             else env.scene["robot"].data.default_joint_pos[0].cpu().tolist()),
        "command_names": env.command_manager.active_terms,
        "observation_names": env.observation_manager.active_terms["policy"],
        "action_scale": (env.action_manager.get_term("joint_pos")._scale.cpu().tolist()
                         if hasattr(env.action_manager.get_term("joint_pos")._scale, 'cpu')
                         else [env.action_manager.get_term("joint_pos")._scale]),
    }

    # Add motion-specific metadata only if motion command exists
